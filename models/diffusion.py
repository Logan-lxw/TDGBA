...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, cond, init_obs, t , distance):
        noise = torch.randn_like(x_start)

        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        x_noisy = apply_conditioning(x_noisy, init_obs, self.observation_dim)

        x_recon = self.model(x_noisy, cond, t, distance)
    ...
    def loss(self, x, cond, init_obs ,distance):
        batch_size = len(x)
        t = torch.randint(0, self.n_timesteps, (batch_size,), device=x.device).long()
        return self.p_losses(x, cond, init_obs, t , distance)
