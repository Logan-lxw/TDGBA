...
class Dataset(ScriptedDataset):
    def __init__(self, task, dataset, human_label_dataset, normalizer, max_len=100, pref_num=500, device="cuda"):
        ...
        self.human_obss_1 = torch.as_tensor(human_label_dataset["observations"], dtype=torch.float32,
                                            device=self.device)[:pref_num]
        self.human_obss_2 = torch.as_tensor(human_label_dataset["observations_2"], dtype=torch.float32,
                                            device=self.device)[:pref_num]
        self.human_actions_1 = torch.as_tensor(human_label_dataset["actions"], dtype=torch.float32,
                                               device=self.device)[:pref_num]
        self.human_actions_2 = torch.as_tensor(human_label_dataset["actions_2"], dtype=torch.float32,
                                               device=self.device)[:pref_num]
        self.timesteps_1 = torch.as_tensor(human_label_dataset["fixed_timestep_1"], dtype=torch.long,
                                           device=self.device)[:pref_num]
        self.timesteps_2 = torch.as_tensor(human_label_dataset["fixed_timestep_2"], dtype=torch.long,
                                           device=self.device)[:pref_num]
        self.preferences = torch.as_tensor(human_label_dataset["labels"], dtype=torch.float32, device=self.device)[
                           :pref_num]
